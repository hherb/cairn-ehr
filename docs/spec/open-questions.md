...
13. **Operational observability without a cloud.** *(Open.)* Running a *fleet* of offline nodes needs a health plane — sync lag, disk pressure, key/credential expiry, hardware throttling, a node that hasn't synced in weeks — surfaced **without any mandatory phone-home** ([principle 7](index.md#founding-principles-the-lens-for-every-decision)). The [§6.2](sync.md#62-consistency-model) freshness indicator and the [§7.10](security.md#710-node-durability-and-disaster-recovery) backup-health fact are the first threads; the open question is the **general mechanism** — likely the same honest-assembly/projection shape, node-local with opportunistic hub aggregation, never a vendor telemetry pipe. Foundational for *operating* Cairn, but safely deferrable until build basics are underway.
14. **Trusted-time anchoring.** *(Open.)* HLC gives causal *ordering* tolerant of skewed wall clocks ([data-model §3.2](data-model.md#32-identity-time)), but not wall-clock *truth*: `t_recorded` is the objective ceiling ([ADR-0003](decisions/0003-bitemporal-time-and-acknowledged-uncertainty.md)) yet a node with a drifting RTC and no NTP can place it arbitrarily, with medico-legal consequences. Open question: how a node **bounds and attests its wall-clock honesty** (e.g. opportunistic time attestation from peers/anchors on reconnect, a recorded clock-confidence grade — acknowledged uncertainty applied to time-of-recording) without a mandatory online time source. Smallest of the current open items.
