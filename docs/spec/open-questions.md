...
14. **Trusted-time anchoring.** *(Open — direction noted.)* HLC gives causal *ordering* tolerant of skewed wall clocks ([data-model §3.2](data-model.md#32-identity-time)), not wall-clock *truth*: `t_recorded` is the objective ceiling ([ADR-0003](decisions/0003-bitemporal-time-and-acknowledged-uncertainty.md)) yet a node with a drifting RTC and no NTP can place it arbitrarily, with medico-legal consequences. **Direction:** a **pluggable, optional timestamping notary** (RFC-3161-style — submit a hash, receive a signed time-stamp token verifiable against the notary's published public key), **defaulting to a self-signed timestamp** when none is configured. Which notary (if any) is the deployment's choice, and self-hosting one is a node role on the [ADR-0017](decisions/0017-federation-admission-sovereignty-peering-and-trust-anchors.md) trust-anchor spectrum; tokens are overlaid signed data, and forward re-notarization before algorithm obsolescence is [ADR-0015](decisions/0015-event-serialization-signatures-and-content-addressing.md) re-attestation-as-overlay. **Open caveats before this is ADR-ready:** (a) a notary bounds `t_recorded` *from above* and only **online** — the offline case (Cairn's whole point) needs **deferred batch notarization** (queue hashes, notarize a Merkle root on reconnect), yielding a loose, honestly-**graded clock-confidence** (principle 4 applied to time), plus causal *lower*-bounding by embedding recently-received anchors; (b) the **self-signed default proves integrity, not external time provenance** — it must be graded as *self-asserted*, never displayed as a trusted timestamp; (c) a single notary is a trust/availability/capture/privacy point (submission-timing traffic analysis) → **multi-notary + self-hostable + Merkle-root submission** (the notary learns *"a batch existed,"* not per-event metadata); (d) the real hardening is the **notary's own time provenance and signing-key protection** plus **long-term token renewal** — the protocol itself is the trivial part. The load-bearing core requirement is that the **clock-confidence grade travels with every event**, so a reader decades on knows whether a `t_recorded` was notary-anchored or self-asserted. The notary is a *source of anchors*, not the whole mechanism.
